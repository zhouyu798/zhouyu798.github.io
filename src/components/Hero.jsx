...
export default function Hero() {
  return (
    <section className="relative min-h-screen overflow-hidden px-5 pt-28">
      ...
      <motion.div
        className="hero-ring left-[58%] top-[10%] h-[420px] w-[420px]"
        animate={{ rotate: 360, y: [0, -18, 0] }}
        transition={{ rotate: { duration: 38, repeat: Infinity, ease: "linear" }, y: { duration: 8, repeat: Infinity, ease: "easeInOut" } }}
      />
      <motion.div
        className="hero-ring -left-24 bottom-12 h-[280px] w-[280px] opacity-40"
        animate={{ rotate: -360, x: [0, 16, 0] }}
        transition={{ rotate: { duration: 44, repeat: Infinity, ease: "linear" }, x: { duration: 7, repeat: Infinity, ease: "easeInOut" } }}
      />
      <div className="relative z-10 mx-auto grid min-h-[calc(100vh-112px)] max-w-canvas items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        >
          <p className="mb-6 inline-flex rounded-full border border-line bg-white/70 px-4 py-2 text-sm font-semibold text-harmony shadow-soft backdrop-blur-xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            {tags.map((tag) => (
              <span key={tag} className="rounded-full border border-line bg-white/72 px-4 py-2 text-sm font-medium text-muted backdrop-blur">
                {tag}
              </span>
            ))}
          </div>
          <div className="mt-10 flex flex-wrap gap-4">
            <a href="#work" className="inline-flex items-center gap-2 rounded-full bg-ink px-6 py-4 text-sm font-semibold text-white shadow-card transition hover:bg-harmony">
              查看作品 <ArrowDown size={18} />
            </a>
            <a href="mailto:2082315263@qq.com" className="inline-flex items-center gap-2 rounded-full border border-line bg-white/78 px-6 py-4 text-sm font-semibold text-ink backdrop-blur-xl transition hover:border-harmony hover:text-harmony">
              联系我 <Mail size={18} />
            </a>
          </div>
        </motion.div>
        ...
        >
          ...
          <motion.div
            className="absolute left-2 top-28 w-[min(88vw,520px)] rounded-system border border-white/75 bg-white/72 p-6 shadow-card backdrop-blur-2xl"
            animate={{ y: [0, -14, 0] }}
          ...
          >
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-2">
              {[
                ...
                <div key={label} className="rounded-[26px] border border-line bg-white/68 p-4">
          ...
          <motion.div
            className="absolute bottom-14 right-4 w-[min(82vw,390px)] rounded-[34px] border border-white/75 bg-white/78 p-5 shadow-card backdrop-blur-2xl"
            animate={{ y: [0, 16, 0] }}
